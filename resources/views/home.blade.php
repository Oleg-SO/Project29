          ...
          <table class="table table-bordered">
            ...
            <tbody>
              @foreach($tasks as $task)
                ...
                <td><?= $task->status === 1 ? '<span class="changeIcon" data-id="'.$task->id.'"><i class="fas fa-clipboard-check fa-2x"></i></span>' : '<span class="changeIcon"  data-id="'.$task->id.'"><i class="fas fa-calendar-times fa-2x"></i></span>' ?></td>
                <td>{{ $task->created_at->diffForHumans() }}</td>
                <td>
                  <form action="{{route('delete',$task->id)}}" method="POST">
                    @csrf
                    @method('delete')
                    <button type="submit" class="btn btn-primary"><i class="fas fa-trash-alt"></i></a>
      ...
      $(".changeIcon").click(function(e){
        ...
        $.ajax({
          ...
          console.log(data.status);
            if(data.status == 1) {
              element.children('i').removeClass('fas fa-calendar-times fa-2x').addClass('fas fa-clipboard-check fa-2x');
            } else {
                element.children('i').removeClass('fas fa-clipboard-check fa-2x').addClass('fas fa-calendar-times fa-2x');
            }
        });
      });
    });
